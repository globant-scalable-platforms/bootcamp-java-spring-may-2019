...
public class ContractRestClientApplicationTest {
	...
	public void get_customer_from_service_contract() {
		...
		// when:
		ResponseEntity<Customer> entity = restTemplate.getForEntity("http://localhost:8100/customers/1", Customer.class);
	...
	@Test
	public void add_customer_from_service_contract() {
		// given:
		RestTemplate restTemplate = new RestTemplate();

		// when:
		ResponseEntity<Customer> entity = restTemplate.postForEntity("http://localhost:8100/customers", new Customer(4L, "Jonnathan", "Sanchez"), Customer.class);

		// then:
		BDDAssertions.then(entity.getStatusCodeValue()).isEqualTo(201);
		BDDAssertions.then(entity.getBody().getId()).isEqualTo(4l);
		BDDAssertions.then(entity.getBody().getName()).isEqualTo("Jonnathan");
		BDDAssertions.then(entity.getBody().getSurname()).isEqualTo("Sanchez");
	}
}
