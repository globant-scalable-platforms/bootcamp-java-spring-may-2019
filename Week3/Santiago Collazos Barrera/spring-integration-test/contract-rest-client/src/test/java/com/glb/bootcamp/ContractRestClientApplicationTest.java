...
public class ContractRestClientApplicationTest {
	...
	public void create_customer_from_service_contract(){
		...
		Customer newCustumer = new Customer(10L,"Santiago","Collazos");

		// when:
		ResponseEntity<Customer> entity = restTemplate.postForEntity("http://localhost:8100/customer/", newCustumer, Customer.class);

		// then:
		BDDAssertions.then(entity.getStatusCodeValue()).isEqualTo(HttpStatus.CREATED_201);
		BDDAssertions.then(entity.getBody().getId()).isEqualTo(newCustumer.getId());
		BDDAssertions.then(entity.getBody().getName()).isEqualTo(newCustumer.getName());
		BDDAssertions.then(entity.getBody().getSurname()).isEqualTo(newCustumer.getSurname());
	}
}
