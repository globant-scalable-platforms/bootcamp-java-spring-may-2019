...
public class ContractRestClientApplicationTest {
	...
	@Test
	public void new_customer_from_service_contract() {
		// given:
		RestTemplate restTemplate = new RestTemplate();
		Customer customer = new Customer(4L, "Diego", "Alvarez");
		
		// when:
		Customer customerResponse = restTemplate.postForObject("http://localhost:8100/customer", customer, Customer.class);

		// then:
		BDDAssertions.then(customerResponse.getId()).isEqualTo(4l);
		BDDAssertions.then(customerResponse.getName()).isEqualTo("Diego");
		BDDAssertions.then(customerResponse.getSurname()).isEqualTo("Alvarez");
