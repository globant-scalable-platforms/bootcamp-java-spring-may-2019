...
public class ContractRestClientApplicationTest {
	...
	@Test
	public void get_customer_from_service_contract2() {
		// given:
		RestTemplate restTemplate = new RestTemplate();

		// when:
		ResponseEntity<Customer> entity = restTemplate.getForEntity("http://localhost:8100/customer/2", Customer.class);

		// then:
		BDDAssertions.then(entity.getStatusCodeValue()).isEqualTo(200);
		BDDAssertions.then(entity.getBody().getId()).isEqualTo(2l);
		BDDAssertions.then(entity.getBody().getName()).isEqualTo("Mauro");
		BDDAssertions.then(entity.getBody().getSurname()).isEqualTo("Perez");
	...
	@Test
	public void get_customer_from_service_contract3() {
		// given:
		RestTemplate restTemplate = new RestTemplate();

		// when:
		ResponseEntity<Customer> entity = restTemplate.getForEntity("http://localhost:8100/customer/3", Customer.class);

		// then:
		BDDAssertions.then(entity.getStatusCodeValue()).isEqualTo(200);
		BDDAssertions.then(entity.getBody().getId()).isEqualTo(3l);
		BDDAssertions.then(entity.getBody().getName()).isEqualTo("Daniel");
		BDDAssertions.then(entity.getBody().getSurname()).isEqualTo("Pelaez");
